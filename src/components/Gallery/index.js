...
class Gallery extends Component {
  ...
  onThumbnailClicked = (imageUrl, imageAltText) => {
    this.setState({ActiveImageUrl: imageUrl, imageAltText})
  }

  render() {
    const {ActiveImageUrl, imageAltText} = this.state
    return (
      <div className="bg-container">
        <img src={ActiveImageUrl} alt={imageAltText} className="custom-img" />
        ...
        <ul className="list-container">
          {imagesList.map(each => (
            <ThumbnailItem
              imageDetails={each}
              key={each.id}
              isActive={ActiveImageUrl === each.imageUrl}
